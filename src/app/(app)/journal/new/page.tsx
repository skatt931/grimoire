  ...
  return (
    <div className="min-h-dvh pt-4 pb-8">
      ...
      <div className="flex items-center gap-3 px-5 mb-4">
        ...
        <h1 className="text-[20px]" style={{ fontFamily: "var(--font-marcellus)", color: "#EAF0F8" }}>
          Нове читання
        </h1>
      </div>
      ...
      <div className="px-5">
        ...
        {step === 1 && (
          <div className="space-y-4 animate-fade-up">
            <div>
              <label className="block text-[10px] font-[600] tracking-[0.2em] uppercase mb-2"
            ...
            <div>
              ...
              <textarea value={question} onChange={(e) => setQuestion(e.target.value)}
                ...
                rows={3}
                className="w-full px-4 py-3 rounded-[14px] text-[13px] outline-none resize-none leading-[1.7]"
            ...
            <button onClick={() => title.trim() && setStep(2)} disabled={!title.trim()}
              className="w-full py-3.5 rounded-[14px] text-[13px] font-[600] tracking-[0.1em] uppercase transition-all disabled:opacity-40"
        ...
        {step === 2 && (
          <div className="space-y-4 animate-fade-up">
            <p className="text-[13px] mb-4" style={{ fontFamily: "var(--font-marcellus)", color: "rgba(234,240,248,.5)" }}>
            ...
            {[
              ...
              <button key={type} onClick={() => { setSpread(type); setStep(3); }}
                ...
                <p className="text-[18px] mb-1" style={{ color: "rgba(139,159,212,.5)" }}>{icon}</p>
                <p className="text-[16px] mb-0.5" style={{ fontFamily: "var(--font-marcellus)", color: "#EAF0F8" }}>{name}</p>
                <p className="text-[12px]" style={{ fontFamily: "var(--font-manrope)", color: "rgba(234,240,248,.4)" }}>{desc}</p>
              </button>
            ))}
          </div>
        )}
        ...
        {step === 3 && (
          <div className="animate-fade-up">
            ...
            <div className="space-y-3">
                ...
                <div key={i} className="rounded-[16px] p-4"
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <input value={pos.name} onChange={(e) => updatePosition(i, { name: e.target.value })}
                      className="text-[11px] font-[600] tracking-[0.12em] uppercase bg-transparent outline-none"
                  ...
                  {pos.card ? (
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-16 rounded-[6px] flex items-center justify-center flex-shrink-0"
                        ...
                        <span className="text-[10px]" style={{ color: pos.card.arcana === "major" ? "rgba(212,168,76,.5)" : "rgba(139,159,212,.5)" }}>✦</span>
                      </div>
                      <div className="flex-1">
                        <p className="text-[14px] mb-0.5" style={{ fontFamily: "var(--font-marcellus)", color: "#EAF0F8" }}>
                          {pos.card.name_uk}
                          {pos.isReversed && <span className="ml-1 text-[10px]" style={{ color: "rgba(212,168,76,.6)" }}>↓</span>}
                        </p>
                        <div className="flex items-center gap-3">
                          ...
                          <button onClick={() => updatePosition(i, { card: null })}
                            className="text-[10px]" style={{ color: "rgba(234,240,248,.25)", fontFamily: "var(--font-manrope)" }}>
                            Змінити
                          </button>
                        </div>
                      </div>
                    ...
                    <button onClick={() => setPickerIndex(i)}
                      className="w-full py-3 rounded-[10px] text-[12px] font-[500] tracking-[0.06em] transition-all"
                      style={{
              ...
              {spreadType === "free_form" && (
                <button onClick={addPosition}
                  className="w-full py-3 rounded-[14px] text-[12px] font-[500] tracking-[0.06em]"
            ...
            <button onClick={() => setStep(4)}
              className="w-full py-3.5 rounded-[14px] text-[13px] font-[600] tracking-[0.1em] uppercase mt-5 transition-all"
              style={{ background: "rgba(139,159,212,.18)", border: "1px solid rgba(139,159,212,.35)", color: "#EAF0F8", fontFamily: "var(--font-manrope)" }}>
              Далі →
            </button>
          </div>
        ...
        {step === 4 && (
          <div className="space-y-4 animate-fade-up">
            ...
            <div className="rounded-[14px] p-4" style={{ background: "rgba(13,11,30,.6)", border: "1px solid rgba(139,159,212,.1)" }}>
              <p className="text-[10px] font-[600] tracking-[0.15em] uppercase mb-2"
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {positions.every((p) => !p.card) && (
                  <span className="text-[11px]" style={{ fontFamily: "var(--font-manrope)", color: "rgba(234,240,248,.3)" }}>Карти не обрані</span>
                )}
              </div>
            </div>
            ...
            <button onClick={save} disabled={saving || !title.trim()}
              className="w-full py-4 rounded-[14px] text-[13px] font-[600] tracking-[0.1em] uppercase transition-all disabled:opacity-40"
