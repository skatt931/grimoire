    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: "rgba(0,0,0,.7)", backdropFilter: "blur(4px)" }}
      ...
      <div className="mt-auto flex flex-col rounded-t-[28px] overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 pb-3 flex-shrink-0">
          <h3 className="text-[18px]" style={{ fontFamily: "var(--font-marcellus)", color: "#EAF0F8" }}>
            Обери карту
          </h3>
        ...
        <div className="px-5 pb-3 flex-shrink-0">
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Пошук..."
            className="w-full px-4 py-2.5 rounded-[12px] text-[13px] outline-none"
        ...
        <div className="flex gap-2 px-5 pb-3 overflow-x-auto flex-shrink-0" style={{ scrollbarWidth: "none" }}>
          {filters.map(({ key, label }) => (
            <button key={key} onClick={() => setFilter(key)}
              className="flex-shrink-0 px-3 py-1 rounded-full text-[10px] font-[600] tracking-[0.08em] uppercase"
              style={{
            ...
            {filtered.map((card) => (
              <button key={card.id} onClick={() => onSelect(card)}
                ...
                <div className="w-full aspect-[2/3] rounded-[6px] flex items-center justify-center"
                  ...
                  {card.arcana === "major"
                    ? <span className="text-[14px]" style={{ color: "rgba(212,168,76,.5)" }}>✦</span>
                ...
                <p className="text-[9px] text-center leading-tight w-full"
